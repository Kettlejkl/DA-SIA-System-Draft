...
class Config:
    ...
    # ── Model Weights ─────────────────────────────────────────────────────────
    YOLO_WEIGHTS       = str(BASE_DIR / "models" / "weights" / "best.pt")
    CRNN_WEIGHTS       = str(BASE_DIR / "models" / "weights" / "crnn_shotor.pth")
    CNN_WEIGHTS        = str(BASE_DIR / "models" / "weights" / "persian_cnn_classifier.pt")

    # ── Detection / Recognition Thresholds ────────────────────────────────────
    YOLO_CONF_THRESHOLD = float(os.getenv("YOLO_CONF", "0.4"))
    CNN_CONF_THRESHOLD  = float(os.getenv("CNN_CONF",  "0.4"))
